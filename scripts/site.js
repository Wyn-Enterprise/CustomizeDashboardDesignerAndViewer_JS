    ...
    if (username === '' || pswd === '') {
        ...
        $.ajax({
            ...
            if (xhr.status === 200) {
                ...
                $.ajax({
                    type: 'GET',
                    url: wynUrl + '/api/dashboards/config?token=' + result.access_token,
                    dataType: 'json',
                    headers: { accept: 'application/json' },
                }).then(function (res, textStatusTwo, xhrTwo) {
                    if (xhrTwo.status === 200) {
                        console.log(res);
                        document.cookie = "accessToken=" + accessToken;
                        document.cookie = "username=" + username;
                        document.cookie = "wynurl=" + wynUrl;
                        document.cookie = "wynversion=" + res.version;
...
function btnSaveDesignerFeaturesClick() {
    let showToolbar = $("#showToolbar")[0].checked ? 'show' : 'hide';
    let showInspector = $("#showInspector")[0].checked;
    ...
    designerFeatures = {
        ...
        showDataBindingPanel: showDataBindingPanel,
        disableChangeDataset: disableChangeDataset,
...
function showDashboards(category) {
    ...
    for (var d = 0; d < dashboards.length; d++) {
        ...
        dashboardCard.innerHTML = `<div class="card-img">
                        <img src="images/` + dbdImageUrl + `" alt="dashboard">
                        </div>
